...
export const CompanyCard: React.FC<CompanyCardProps> = ({ company, index = 0 }) => {
  ...
  return (
    <Link href={`/companies/${company.id}`} className="block h-full">
      <article 
        className="bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 p-6 border border-gray-200 hover:border-primary cursor-pointer h-full flex flex-col opacity-0 animate-fade-in focus-within:ring-2 focus-within:ring-primary focus-within:ring-offset-2"
        style={{ 
      ...
      >
        <header className="flex items-start justify-between mb-4 gap-3">
          <h3 id={`company-${company.id}-title`} className="text-xl font-semibold text-primary flex-1 min-w-0">
            {company.name}
          </h3>
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex justify-between items-center mb-4">
            <span className="text-sm text-secondary">Total Shares:</span>
          ...
          <div className="flex-1">
            {company.shareholders.length > 0 ? (
              <div>
                <p className="text-sm text-secondary mb-2">Top Shareholders:</p>
                  ...
                  {company.shareholders
                    ...
                    .map((shareholder) => (
                      <div key={shareholder.id} className="flex justify-between text-sm">
                        <span className="text-secondary font-medium truncate pr-2">{shareholder.name}</span>
                        <span className="text-secondary flex-shrink-0">{shareholder.shares.toLocaleString()}</span>
                      </div>
                    ))}
                </div>
              </div>
        ...
        <footer className="mt-4 pt-4 border-t border-gray-100">
          <div className="flex items-center text-primary text-sm font-medium transition-colors">
